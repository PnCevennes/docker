. config/settings.ini
  ...
  sed -i "s/URL_APPLICATION = .*$/URL_APPLICATION = '${my_url}:5000' /g" config/geonature_config.toml
  sed -i "s/API_ENDPOINT = .*$/API_ENDPOINT = '${my_url}:5000\/api'/g" config/geonature_config.toml
  sed -i "s/API_TAXHUB = .*$/API_TAXHUB = '${my_url}:5002\/api'/g" config/geonature_config.toml
